...
is byte-identical.** Eval reach: the eval profile names `qwen3.5-4b` explicitly,
so the five new entries are inert for eval; the fallback engine pin moves to
b11146 for fresh installs only — an existing jobfaro engine is never replaced
...
is byte-identical.** Eval reach: eval postings are text-only, and the eval
profile's serve heals a projector like any launch — inert for eval results,
and a jobfaro backend used interactively gains image input.
...
## 1.37.0-jobdar.1 — 2026-08-07 (winc-jobdar branch)

Merge of master **v1.37.0** (`winc restart` / `winc stop`). **`internal/cli/eval.go`
is byte-identical.** Eval reach: the `:8080` eval serve now records the pidfile
too, so a jobfaro backend can be bounced with `winc restart` — the recorded
invocation replays `--eval` and the model exactly.
...
**`internal/cli/eval.go` is byte-identical.** The version-literal gate test now
covers this branch too: its newest CHANGELOG heading must match the branch's
own `-jobdar.N` Version literal, so a forgotten bump fails the gate here as
well. DFlash reach into eval: the eval profile's `:8080` serve names Qwen
models explicitly — if a DFlash head has been downloaded for one, the serve
picks it up like any launch; drafts are target-verified, so eval outputs are
unaffected.
...
priority on non-unified machines). **The eval profile is untouched** —
`internal/cli/eval.go` is byte-identical, and the change lives entirely in
team mode (`team.go` worker launch + `internal/server` priority helpers),
which the `:8080` eval serve never enters.
...
default). **The eval profile is untouched** — `internal/cli/eval.go` is
byte-identical, and the change is catalogue-order + fresh-install-template
only. The eval tiers (`qwen3.5-4b`/`2b`/`gemma4-e2b`) live in the `nano`
tier, and `evalPickModel` selects by its own tier list, not by small-tier
...
## 1.29.0-jobdar.1 — 2026-07-24 (winc-jobdar branch)

Merge of master **v1.29.0** (metadata-first handler pass + two small-tier
catalogue additions). **The eval profile is untouched** — `internal/cli/eval.go`
is byte-identical to jobdar.3, and master's two new launch-path guards are
inert for it by construction:

- The **trained-context clamp** caps a window at `<arch>.context_length`. The
  eval profile pins **16384**, far below every eval-tier model's trained length
  (qwen3.5-4b/2b 262144, gemma4-e2b 131072), so the clamp never engages and the
  `-c` value is unchanged.
- The **draft tokenizer check** only runs when `[performance].draft_model` is
  set; the eval profile leaves speculative decoding off, so the branch never
  reaches it.
- **MoE detection** now reads `<arch>.expert_count`; every eval-tier model is
  dense (expert_count absent → 0 → dense), the same answer the old filename
  heuristic gave.

Inherited from master and useful here: `gemma4-e2b`'s real trained context is
**131072**, not the 262144 winc used to advertise — worth knowing for any
eval-tier sizing assumption, though the 16384 pin means nothing changes today.

Also inherits the two new `small`-tier models (`lfm2.5-8b-a1b`, `ornith-9b`);
neither is an eval-tier pick, so `evalPickModel` is unaffected.
...
loss; ubatch-flat + f16-KV-validated CUDA notes) into the jobdar stability
branch -- see the entry below. The eval profile is UNAFFECTED: `--eval`
already ran draft/MTP off and pins `cache_type = "q8_0"` explicitly; the
retirement removes only the general auto-pair path serving/agent flows used.
...
## 1.27.0-jobdar.1 — 2026-07-17 (winc-jobdar branch)

Merges master **v1.26.0** (context pin) + **v1.27.0** (the speed pass) into
the jobdar stability branch -- see those entries below. The eval profile is
UNAFFECTED by the speed pass: it pins `cache_type = "q8_0"` explicitly (so the
new f16 auto-policy never applies to scoring), already runs draft/MTP off, and
uses no team mode. `--cache-reuse` removal reaches the eval serve too (it was
the measured-no-gain flag there as well).
...
## 1.25.0-jobdar.1 — 2026-07-16 (winc-jobdar branch)

Merge master v1.24.0 + v1.25.0: the journal (context virtualization) and its
speed pass land on the branch. The eval profile is guarded, not journaled.

### Changed
- **`--eval` forces the journal off.** Eval requests are single-shot — there
  is nothing to virtualize, and each request would otherwise become a junk
  conversation in the store (the default is now journal-ON for small
  windows). An explicit `--eval --journal` is rejected with the reason; the
  config default is silently overridden with a status note. The eval
  profile's own behavior (pinned port, reasoning off, greedy, grammar-JSON,
  tier presets) is byte-unchanged by this merge.
- Everything else inherits from master v1.24.0/v1.25.0 — see those entries
  below: journal on-by-default with the <48k dormancy gate, BM25 recall +
  async rolling summary, `winc journal` CLI, `cmd/journalbench`.
...
engine assets (WoA Adreno OpenCL + cpu, ARM Linux vulkan + cpu), P-core
thread pinning for cpu-backend inference, and the ARM-CPU Q4_0 catalog rungs
-- see the v1.23.0 entry below. The eval profile is unchanged in this merge;
the branch-side low-tier eval preset ships separately as 1.23.0-jobdar.2.

## 1.22.0-jobdar.1 — 2026-07-09 (winc-jobdar branch)

Merges master **v1.22.0** into the jobdar stability branch: per-push CI (which
now covers THIS branch's pushes and merge commits -- the gap that let the
branch-only gofmt violation live for weeks), If-Range-validated resume, the
30s stall watchdog, model sha256 verification, download tests, and Go 1.26 --
see the v1.22.0 entry below. The jobdar eval profile (`winc serve --eval`) is
unchanged; its model bootstrap (`promptDownloadEvalModel` -> `HFDownloadAs`)
inherits the verified-download path automatically.
## v1.23.0 — 2026-07-09
...
`qwen3.6-27b-q4-mtp` catalog rung (see the v1.21.5 entry below). The jobdar eval
profile (`winc serve --eval`) is unchanged.
...
summaries no longer return empty on default-thinking-on models (Qwen3.6) -- see the
v1.21.4 entry below. The jobdar eval profile (`winc serve --eval`) is unchanged.
